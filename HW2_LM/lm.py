...
class LangModel:
    ...
    def entropy(self, corpus):
        ...
        sum_logprob = 0.0
        for s in corpus:
            num_words += len(s)-2  #sentence involved 2 sos and 1 eos so remove 2 for sos which is not involved in prob calculation.
...
class Trigram(LangModel):
    ...
    def logprob_sentence(self, sentence):
        ...
        for i in xrange(2,len(sentence)):
            ...
            #p += self.conditional_log2_probability(sentence[i], sentence[:i])
        return p
    ...
    def conditional_log2_probability(self, word, previous):
        UV = previous[-2] + " " + previous[-1]
        UVW = previous[-2] + " " + previous[-1] + " " + word
